void check_gps(void) {
  ...
  while (loop_counter < 1000) {                                                           //Stay in this loop until the data variable data holds a q.
    ...
    if (loop_counter > 1 && loop_counter < 500)while (ss.available())Serial.print((char)ss.read());
    if (loop_counter == 500) {
      Serial.println("");
      Serial.println("====================================================================");
    ...
    if (loop_counter > 500 && loop_counter < 1000)while (ss.available())Serial.print((char)ss.read());
