...
int temperature;
long acc_x, acc_y, acc_z, acc_total_vector[20], acc_av_vector;
int acc_axis[4], gyro_axis[4];
double gyro_pitch, gyro_roll, gyro_yaw;
float angle_roll_acc, angle_pitch_acc, angle_pitch, angle_roll;
int cal_int, loop_counter;
...
//      PORTD &= B00001111;                                                               //Set digital poort 4, 5, 6 and 7 low.
      ...
      if(loop_counter == 1)printf(angle_pitch ,0);
      if(loop_counter == 2)printf(" Roll: ");
      if(loop_counter == 3)printf(angle_roll ,0);
      if(loop_counter == 4)printf(" Yaw: ");
      if(loop_counter == 5)printf(gyro_yaw / 65.5 ,0);
      printf("\n");
