...
//!   1. Reset
//!   2. Read PROM (128 bit of calibration words -> 6x16 bits for C1-C6 (96 bit total) + 32)
//!   3. D1 conversion  (Digital pressure value)
//!   4. D2 conversion  (Digital temperature value)
//!   5. Read ADC result (24 bit pressure / temperature)
//! 
//! First we have to read the PROM for callibration
//! Then we have to read the digital pressure and temperature


//! ______ C A L C U L A T E    T E M P E R A T U R E
//! 
//! Difference between actual and reference temperature (dT)
//! dT = D2 - T_ref   =>   D2 - C5 << 8
//! 
//! Calculate actual temperature (TEMP) 20 degrees with 0.01 resolution
//! TEMP = 20 + dT*TEMPSENS   =>   2000 + dT* C6 >> 23

//! ______ C A L C U L A T E    T E M P E R A T U R E    C O M P E N S A T E D    P R E S S U R E
//! 
//! Offset at actual temperature
//! OFF = OFF_T1 + TCO*dT   =>   C2 << 16 + (C4 * dT) << 7
//! 
//! Sensitivity at actual temperature (SENS)
//! SENS = SENS_T1 + TCS*dT   =>   C1 >> 15 + (C3*dT) << 8
//! 
//! Temperature compensated pressure (10 - 1200mbar with 0.01mbar resolution)
//! P = D1 * SENS - OFF   =>   (D1 * SENS >> 21 - OFF) >> 15
//! 
//! ______ R E T U R N    P R E S S U R E    &    T E M P E R A T U R E    V A L U E
...
//! 
//! The start condition is that both the SDA and SCL goes
//! to 0. Then follows the 7 address bits (i.e. 0x77 -> 0x1110111)
//! 
//! Then follows a W/R bit (1=W, 0=R), saying if the master wants to read or write
//! From there the slave sends an ACK bit (0), as an acknowledgement
//! 
//! This is followed by an 8 bit msg (always 1 byte)
//! which is followed by an ack from the counterpart
//! 
//! The communication stops with the stop bit (both SCL and SDA goes high)
//! 
//! @return 0
//********************************************************
unsigned char i2c_start(unsigned char address)
